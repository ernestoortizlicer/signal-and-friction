...
-- ── MODULE 05 ──
('M05', 1, '05.A',
 'ChatGPT, Claude, and Gemini can review a funnel and generate a friction hypothesis in minutes. Within 24 months, every B2B SaaS founder will have AI tools capable of producing a diagnostic document at zero marginal cost. High-ticket consulting fees are therefore on a structural decline curve. Why train practitioners to charge $2,800 for a deliverable that AI will soon produce for $0?',
 'This objection conflates output generation with clinical judgment. An AI tool can produce a document that resembles a friction hypothesis. It cannot run a live PostHog session recording audit against a specific client''s funnel, isolate a causal variable from a confounded dataset, apply the Isolation Gate Framework under time pressure, and deliver a signed intervention recommendation with a practitioner''s professional reputation behind it. The practitioner''s value is not the document — it is the signature on the document. Clients paying $2,800 are buying accountability, not text.',
