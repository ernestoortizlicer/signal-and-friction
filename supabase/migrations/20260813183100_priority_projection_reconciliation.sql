...
BEGIN
  ...
  IF p.status IN ('closed_completed','closed_lost') THEN UPDATE public.priority_tasks SET status='eliminated',updated_at=now() WHERE source_table='beta_projects' AND source_id=p_project_id AND auto_generated=true AND status IN ('pending','in_progress'); RETURN NULL; END IF;
  ...
  PERFORM public.calculate_priority_score(new_id); RETURN new_id;
END $$;

CREATE OR REPLACE FUNCTION public.sync_beta_project_to_priority_tasks() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO '' AS $$ BEGIN PERFORM public.ensure_project_priority_task(NEW.id,true); RETURN NEW; END $$;
...
BEGIN
  SELECT * INTO i FROM public.ai_incidents WHERE id=p_incident_id; IF i.id IS NULL THEN RETURN NULL; END IF;
  IF i.resolved_at IS NOT NULL OR i.severity NOT IN ('high','critical') THEN UPDATE public.priority_tasks SET status='done',completed_at=COALESCE(completed_at,now()),updated_at=now() WHERE source_table='ai_incidents' AND source_id=p_incident_id AND auto_generated=true AND status IN ('pending','in_progress'); RETURN NULL; END IF;
...
CREATE OR REPLACE FUNCTION public.sync_incident_to_priority_tasks() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO '' AS $$ BEGIN PERFORM public.ensure_incident_priority_task(NEW.id,true); RETURN NEW; END $$;
