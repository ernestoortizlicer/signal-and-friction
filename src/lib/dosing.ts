 ...
 *
 * DUPLICATED from supabase/functions/_shared/dosing.ts, not imported —
 * that file lives in the Deno edge-function tree; this Next.js app and
 * the Deno functions are separate build systems with no shared module
 * resolution path between them (same constraint already documented in
 * functions/api/diagnose.ts for linguistic-sandbox.ts). Keep both copies
