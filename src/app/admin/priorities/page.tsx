    ...
    <main className="min-h-screen bg-[#0A0908] text-[#B8B0A8] p-8 md:p-12 grain overflow-x-hidden">
      <div className="max-w-[1400px] mx-auto space-y-10">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between border-b border-[#D4A853]/8 pb-8">
          <div>
            <span className="font-mono text-xs uppercase tracking-[0.2em] text-[#9A8F82] block mb-2">Decision Matrix OS</span>
        ...
        <section className="grid grid-cols-2 lg:grid-cols-5 gap-4">
          {[
            ...
            { label: "Avg Priority", value: pendingTasks.length > 0 ? (totalScore / pendingTasks.length).toFixed(1) : "—", detail: "Score /100", color: "text-[#D4A853]" },
            { label: "Revenue at Risk", value: `$${(pendingTasks.reduce((s, t) => s + t.revenue_impact, 0) / 100).toLocaleString()}`, detail: "Pending tasks", color: "text-[#D4A853]" },
            { label: "Today Effort", value: `${filteredTasks.reduce((s, t) => s + t.effort_minutes, 0)} min`, detail: `${(filteredTasks.reduce((s, t) => s + t.effort_minutes, 0) / 60).toFixed(1)} hours`, color: "text-[#9A8F82]" },
          ].map((item, idx) => (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-3 gap-3 mb-4 text-center">
                        <div className="bg-black/30 border border-[#D4A853]/8 rounded py-2 px-1">
                          <span className="font-mono text-xs text-[#9A8F82] block uppercase">Score</span>
                        ...
                        <div className="bg-black/30 border border-[#D4A853]/8 rounded py-2 px-1">
                          <span className="font-mono text-xs text-[#9A8F82] block uppercase">Effort</span>
                          <span className="font-serif text-lg font-bold text-[#F5F0EB]">{task.effort_minutes}m</span>
                        </div>
                        <div className="bg-black/30 border border-[#D4A853]/8 rounded py-2 px-1">
                          <span className="font-mono text-xs text-[#9A8F82] block uppercase">Energy</span>
                      ...
                      <tbody>
                        {filteredTasks.slice(3).map(task => (
                          <tr key={task.id} className="border-b border-[#D4A853]/8 hover:bg-white/[0.01]">
                            ...
                            <td className="text-right text-[#9A8F82]">{task.deadline ? new Date(task.deadline).toLocaleDateString() : '—'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                              ...
                              <AnimatePresence>
                                {selectedTask === task.id && (
                                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                                    <div className="border-t border-[#D4A853]/8 mt-3 pt-3 text-xs font-mono text-[#9A8F82] space-y-1">
                                      {task.description && <p className="italic">{task.description}</p>}
                                      <p>Learning: {task.learning_multiplier}/10 · Deadline: {task.deadline ? new Date(task.deadline).toLocaleDateString() : 'None'}</p>
            ...
            <motion.div key="dayplan" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={springConfig} className="space-y-6">
              <div className="border border-[#D4A853]/8 p-6 bg-[#121110]/20 rounded">
                <h3 className="font-serif text-xl text-[#F5F0EB] mb-6">
                  📅 Optimized Day Plan — {new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}
                </h3>
                        ...
                        <div className="flex gap-6">
                          ...
                          <div className="flex-1 space-y-3">
                            <div className="flex items-center gap-3 mb-3">
                              ...
                              <h4 className="font-mono text-xs uppercase tracking-wider text-[#F5F0EB]">{block.label} Block</h4>
                ...
                <div className="border-t border-[#D4A853]/8 mt-8 pt-4 flex justify-between text-xs font-mono text-[#9A8F82]">
                  <span>Total planned: {filteredTasks.reduce((s, t) => s + t.effort_minutes, 0)} min ({(filteredTasks.reduce((s, t) => s + t.effort_minutes, 0) / 60).toFixed(1)} hours)</span>
                  <span>{filteredTasks.length} tasks across {Object.values(energyBlocks).filter(b => b.tasks.length > 0).length} energy blocks</span>
                </div>
              </div>
            </motion.div>
          )}
            ...
            <motion.div key="autopilot" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={springConfig} className="space-y-6">
              ...
              <div className="border border-[#D4A853]/8 p-6 bg-[#121110]/20 rounded">
                ...
                <p className="text-sm text-[#9A8F82] font-mono mb-6">
                  Select your available time. The engine will select the optimal combination of tasks that maximizes total priority value within your time window.
                </p>

                <div className="flex items-center gap-4 mb-6">
                  <span className="font-mono text-xs text-[#9A8F82] uppercase">Available time:</span>
                ...
                <div className="space-y-2 mb-4">
                  <div className="flex items-center justify-between text-xs font-mono">
                    <span className="text-[#9A8F82]">Selected: {autoPilotSelection.length} tasks · {autoPilotSelection.reduce((s, t) => s + t.effort_minutes, 0)} min used</span>
                    <span className="text-[#B85C38]">Priority value: {autoPilotSelection.reduce((s, t) => s + Number(t.priority_score), 0).toFixed(0)} pts</span>
                  </div>
                  ...
                  <div className="flex justify-between text-xs font-mono text-[#7A6F65]">
                    <span>0 min</span>
                    <span>{remaining} min buffer remaining</span>
              ...
              {autoPilotSelection.length === 0 && (
                <div className="text-center text-[#9A8F82] font-mono text-xs py-12 border border-[#D4A853]/8 rounded bg-[#121110]/20">
                  No tasks fit within {autopilotMinutes} minutes. Try a larger time block.
                </div>
              )}
            </motion.div>
          )}
